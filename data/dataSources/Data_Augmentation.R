...
positive_scalar <- 20   # Amplify positive examples  20 times
test_percent <- 0.1


#pair_file_name <- paste(celline, "/", celline, ".csv", sep = "")
pair_file_name <- paste(root, celline, ".csv", sep = "")   # "data/dataSources/IMR90.csv"
print(pair_file_name)

############################################################################################################
#balanced training
enhancer_file_name <- paste(root, celline, "/", folder, "/", celline, "_enhancer.fasta", sep = "")
promoter_file_name <- paste(root, celline, "/", folder, "/", celline, "_promoter.fasta", sep = "")
label_file_name <- paste(root, celline, "/", folder, "/", celline, "_label.txt", sep = "")
enhancer_coor_file_name <- paste(root, celline, "/", folder, "/", celline, "_enhancer_coor.txt", sep = "")
promoter_coor_file_name <- paste(root, celline, "/", folder, "/", celline, "_promoter_coor.txt", sep = "")
enhancer_coor_file_name_stat <- paste(root, celline, "/", folder, "/", celline, "_enhancer_coor_stat.txt", sep = "")
promoter_coor_file_name_stat <- paste(root, celline, "/", folder, "/", celline, "_promoter_coor_stat.txt", sep = "")
enhancer_bed_file_name <- paste(root, celline, "/", folder, "/", celline, "_enhancer.bed", sep = "")
promoter_bed_file_name <- paste(root, celline, "/", folder, "/", celline, "_promoter.bed", sep = "")

#test
enhancer_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_enhancer_test.fasta", sep = "")
promoter_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_promoter_test.fasta", sep = "")
label_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_label_test.txt", sep = "")
enhancer_coor_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_enhancer_coor_test.txt", sep = "")
promoter_coor_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_promoter_coor_test.txt", sep = "")
enhancer_bed_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_enhancer_test.bed", sep = "")
promoter_bed_file_name_test <- paste(root, celline, "/", folder, "/", celline, "_promoter_test.bed", sep = "")


#imbalanced training
im_folder <- paste(root,celline, "/", folder, "/imbalanced/", sep = "")  # "data/dataSources/",celline, "/aug/imbalanced/"
print(paste("imbalanced training folder:", im_folder))
if (!file.exists(im_folder)) {
  dir.create(im_folder, recursive = TRUE)
  print("created im floder!")
}else{
  print(paste(im_folder, "already exists"))
}
...
adjustment <- function(chr_end, start, end, target_len) {
  ...
  if (str_len < target_len) {
    # 如果en、pr序列duan原始长度小于目标长度，则在两端添加相同长度补充，并记录新的start和end
    # start_update is idx
    ...
    # 得到的新的结束下标（end_update）在序列之外，则整体前移
    if (end_update > chr_end) {
      end_update <- chr_end
      start_update <- end_update + 1 - target_len
    } #if

    # coor represents coordinates:
    # start_coor 左端补充的序列长度
    # end_coor
...
interaction_positive <- which(interaction == 1)   # 返回interaction中等于1的变量值得位置
#print(interaction_positive)
interaction_negative <- which(interaction == 0)
#print(interaction_negative)
...
interaction_test <- interaction[test_index]   # get the real value from interaction by index
enhancer_list_test <- enhancer_list[test_index]   # get the name list of enhancer
promoter_list_test <- promoter_list[test_index]   # get the name list of promoter
...
for (i in 1:length(interaction_test)) {   # iterate enhancer_name by enhancer_list_test
  loc <- strsplit(enhancer_list_test[i], split = "|", fixed = T)[[1]][2]      # get "chr1:9933200-9933400"
  chr <- strsplit(loc, split = ":")[[1]][1]   # get "chr1"
  segment <- strsplit(loc, split = ":")[[1]][2]   # get "9933200-9933400"
  start <- strsplit(segment, split = "-")[[1]][1]   # get 9933200
  end <- strsplit(segment, split = "-")[[1]][2]   # get 9933400
  start <- as.numeric(start)    # type transform to be bumber
  end <- as.numeric(end)
...
#z-score
write.table(rbind(c(mean(starts_coor), sd(starts_coor)),
                  c(mean(ends_coor), sd(ends_coor))),
            file = enhancer_coor_file_name_stat,
